...
var Sticky = function (_Component) {
  ...
  function Sticky() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Sticky.__proto__ || Object.getPrototypeOf(Sticky)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
      ...
      var calculatedWidth = contentClientRect.width;

      var bottomDifference = distanceFromBottom - _this.props.bottomOffset - calculatedHeight;
      var topDifference = distanceFromTop - _this.props.topOffset - calculatedHeight;

      var isTopSticky = distanceFromTop <= -_this.props.topOffset && distanceFromBottom > -_this.props.bottomOffset;

      var isBottomSticky = distanceFromTop <= -_this.props.topOffset && distanceFromBottom > -_this.props.bottomOffset;

      var wasSticky = !!_this.state.isSticky;
      var getIsSticky = function getIsSticky() {
        return _this.props.bottom ? isBottomSticky : isTopSticky;
      };
      var isSticky = preventingStickyStateChanges ? wasSticky : getIsSticky();
      ...
      var stickyStyles = {
        ...
        bottomStickyStyle: {
          bottom: bottomDifference - windowHeight > 0 ? _this.props.relative ? parent.offsetTop - parent.offsetParent.scrollTop : 0 : bottomDifference - windowHeight
        },
        bottomAnchorStyle: {
          bottom: bottomDifference - windowHeight + calculatedHeight > 0 ? _this.props.relative ? parent.offsetTop - parent.offsetParent.scrollTop : 0 : -(bottomDifference - windowHeight) - calculatedHeight
        }
      };

      var isBottomAnchor = _this.props.bottomAnchor;
      var isBottom = _this.props.bottom;
      var getBottomStyle = function getBottomStyle() {
        if (isBottom) return stickyStyles.bottomStickyStyle;
        if (isBottomAnchor) return stickyStyles.bottomAnchorStyle;
      };

      var stickyStyle = isBottom || isBottomAnchor ? getBottomStyle() : stickyStyles.topStickyStyle;

      var style = !isSticky ? {} : _extends({}, baseStickyStyle, stickyStyle);

      if (_this.props.mobileStyle) {
        var isMobileView = windowWidth <= _this.props.mobileStyle.width;
        if (isMobileView) {
          style = _extends({}, baseStickyStyle, stickyStyles[_this.props.mobileStyle.style + "Style"]);
        }
      }
  ...
  _createClass(Sticky, [{
    ...
    value: function componentDidUpdate() {
      ...
      this.placeholder.style.paddingRight = this.props.disableCompensation ? 0 : (this.state.isSticky ? this.state.calculatedWidth : 0) + "px";
    }
  }, {
    key: "render",
...
Sticky.propTypes = {
  ...
  bottomAnchor: _propTypes2.default.bool,
  mobileStyle: _propTypes2.default.shape({
